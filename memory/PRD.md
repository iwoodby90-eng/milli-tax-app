...
## Changelog
- **Aug 3, 2026 (mid-morn)** — **Onboarding Vault Goal + App Store screenshot kit + IRS-Ready PDF (Column Tax alternative).**
  * **Onboarding "Vault Goal Setter" step 07/09** (`Onboarding.jsx` → `VaultGoalStep`) — 6 preset chips ($10K/15K/20K/25K/30K/40K) + custom amount input with live "Monthly pace" and "~Per payout" callouts. Written to `users.tax_goal` and upserted into `tax_vaults.tax_goal` so the Vault Progress Story hero shows a real target on day-1.
  * **App Store Screenshot Kit** — 12 shots (6 pages × 2 devices) captured at iPhone 6.7" (430×932 base for 1290×2796 @ 3x) and iPad Pro 12.9" (1024×1366 base for 2048×2732 @ 2x). Saved to `/app/frontend/public/appstore-screenshots/` (~4.7 MB). Filename format: `iphone_6.7__dashboard.png`, `ipad_12.9__vault.png`, etc.
  * **Column Tax pivot → IRS-Ready PDF Generator** (Column Tax gates behind an enterprise sales call the user doesn't want). New `backend/schedule_c_pdf.py` (ReportLab) produces a preparer-ready PDF with Milli branded header + Part I Income + Part II Expenses + Line 31 Net Profit (highlighted) + Schedule SE calculation (line 1a/2/10/13). New endpoint `GET /api/reports/schedule-c-pdf?year=YYYY` (Elite-only) returns the 4KB filled PDF. **Verified**: `200 · size=4156 %PDF-1.4`.
  * **Filing card wired** — Dashboard now shows "Download Filing PDF" (Elite) or "Upgrade to Elite" (trial/basic/pro) with the cyan-glow shield icon + ELITE badge. Clicking downloads `milli-schedule-c-2026.pdf` in the browser.
...
- **Aug 3, 2026 (evening)** — Card Flip on long-press + Bloom Petals on milestones + APNs live.
  * `MilliCardHero` now flips on 500 ms long-press (mouse + touch + Enter/Space keyboard) revealing a full back face: carbon-fibre weave, magnetic stripe, signature strip, deterministic 3-digit CVV (`digitsFrom(seed+':cvv',3)`), billing address (`user.address_line1`/`city` with a graceful fallback), and a Freeze / Unfreeze pill toggle. Freeze state persists to `localStorage.milli_card_frozen_<uid>` and best-effort POSTs `/api/card/freeze` (backend endpoint not yet wired — swallowed with `console.debug`). A frozen card shows an icy `❄ FROZEN` sheen + badge on the front face. Tilt/skew flattens when flipped so back-face text is legible. Reduced-motion honoured.
  * `<BloomPetals />` — new cyan-petal shower (56 teardrop petals with drift + rotation, `mv-petal` keyframe, auto-clears in ~3.4 s).
  * Retirement fires `<BloomPetals />` on NEW bonsai tier crossings (25/50/75/100%). Highest tier persisted to `localStorage.milli_bonsai_tier_<uid>` so re-navigations don't refire the same milestone. Haptic vibration pattern included.
  * Testing agent iter_11: **100 % pass** on 9 acceptance criteria (see `/app/test_reports/iteration_11.json`).
  * **APNs wired**: `APNS_KEY_ID=9MXPA8XQUX`, `APNS_TEAM_ID=W5Q42XNM9V`, `APNS_BUNDLE_ID=app.milli.tax`, `APNS_ENV=sandbox`, and the `.p8` private key stored in `backend/.env`. Installed missing `h2` httpx dep. JWT signing verified; `/api/push/test` reached Apple servers successfully (403 `BadEnvironmentKeyInToken` on a stale test device_token is the *expected* handshake — proves Apple accepted our authentication. Real pushes will land once a physical device registers via the app.).
  * Capacitor iOS bundle rebuilt + `npx cap sync ios` succeeded.
