...
- **Aug 3, 2026 (evening)** — Card Flip on long-press + Bloom Petals on milestones.
  * `MilliCardHero` now flips on 500 ms long-press (mouse + touch + Enter/Space keyboard) revealing a full back face: carbon-fibre weave, magnetic stripe, signature strip, deterministic 3-digit CVV (`digitsFrom(seed+':cvv',3)`), billing address (`user.address_line1`/`city` with a graceful fallback), and a Freeze / Unfreeze pill toggle. Freeze state persists to `localStorage.milli_card_frozen_<uid>` and best-effort POSTs `/api/card/freeze` (backend endpoint not yet wired — swallowed with `console.debug`). A frozen card shows an icy `❄ FROZEN` sheen + badge on the front face. Tilt/skew is flattened when flipped so back-face text is legible. Reduced-motion honoured. Testids: `<testid>-front`, `<testid>-back`, `<testid>-cvv`, `<testid>-billing`, `<testid>-freeze-toggle`, `<testid>-frozen-badge`.
  * `<BloomPetals />` — new cyan-petal shower (`/app/frontend/src/components/BloomPetals.jsx`), 56 teardrop-shape petals with drift + rotation, keyframe `mv-petal`, auto-clears in ~3.4 s.
  * Retirement fires `<BloomPetals />` when Jordan crosses a NEW bonsai tier (25 / 50 / 75 / 100 %). Highest tier is persisted in `localStorage.milli_bonsai_tier_<uid>` so re-navigations don't refire the same milestone. Includes haptic vibration pattern on native.
  * Testing agent iter_11: **100 % pass** on 9 acceptance criteria (see `/app/test_reports/iteration_11.json`). Capacitor iOS bundle rebuilt + `npx cap sync ios` succeeded.
