...
| Allowed origins | `CORS_ORIGINS` | Implemented; restrict in production |
...
Current state: Link token creation, public-token exchange, item storage, transaction sync, and gig-deposit classification exist in `backend/server.py`. Production access and a public HTTPS webhook are still required.
...
Current state: Subscription checkout code exists. The two API-key variables currently refer to the same Stripe secret key because separate modules use different variable names. They should be consolidated in a later refactor.
...
Current state: Card-order modules exist, but they were written for the legacy PostgreSQL data layer and are **not production-ready with the current MongoDB backend**. Do not expose card-order routes until the storage layer is rewritten and KYC/compliance requirements are approved.
...
Current state: The current assistant uses the Emergent integrations client. A direct OpenAI or Gemini provider can replace this later if preferred.
...
| Receipt OCR | Google Cloud Vision or a selected OCR provider | Environment placeholder exists; production credential and storage flow must be finalized |

## 4. Recommended credential order

1. MongoDB production connection and a strong JWT secret.
2. Plaid Sandbox credentials.
3. Stripe test secret and webhook secret.
4. Apple APNs key and App ID capabilities.
5. Milli AI provider credential.
...
- Never commit `.env`, `.p8`, service-account JSON, private keys, or webhook secrets.
...
- Restrict CORS, webhook origins, and provider permissions before launch.
