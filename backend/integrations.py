...
Current concrete implementations (all internal/demo):
    banking        → InternalLedgerBank (Milli's own DB-backed ledger)
...
class MoneyMover(Protocol):
    ...
    async def credit(self, user_id: str, bucket: str, amount: float,
                      note: str, source_ref: Optional[str]) -> float: ...
...
class NullTaxPayer:
    ...
    async def submit_state(self, user_id, amount, period, state):
        return {"status": "pending_partner", "amount": amount, "state": state}
...
class InAppNotifier:
    """Persist notifications to db.notifications; UI polls or subscribes later."""
    def __init__(self, db):
        self.db = db
